...
public:
    ...
    {
        if ((!head) || (head->item == elem)) { return addFirst(item); }
        else
        {
            ListItem<T>* tmp = nullptr;
            ListItem<T>* h   = nullptr;
            tmp              = head;
            while ((tmp->next) && (elem != tmp->next->item)) tmp = tmp->next;
            h = tmp->next;
            if (h)
            {
    ...
    {
        if ((!tail) || (tail->item == elem)) { return addLast(item); }
        else
        {
            ListItem<T>* tmp = nullptr;
            ListItem<T>* h   = nullptr;
            tmp              = tail;
            while ((tmp->prev) && (elem != tmp->prev->item)) tmp = tmp->prev;
            h = tmp->prev;
            if (h)
            {
    ...
    int trim(int n)
    {
        T*  t = 0;
        int i = 0;
        for (; (i < n) && ((t = removeLast()) != 0); i++) { delete t; }
        return i;
    }
...
public:
    ...
    int isNext()
    {
        if ((cursor) && (cursor->next)) return 1;
        return 0;
    }

    int isPrev()
    {
        if ((cursor) && (cursor->prev)) return 1;
        return 0;
    }
...
public:
    ...
    T* overwriteNth(int n, T* t) { return content.overwriteNth(n, t); }
...
public:
    ...
    T* get_next()
    {
        if ((list) && (cursor))
        {
            Pix x = cursor;
            list->content->next(x);
            if (x) return (T*)list->content->contents(x);
        }
        return 0;
    }
...
public:
    ...
    T& operator[](int n) const { return *(content[n]); }
...
public:
    ...
    T* add(T* item) { return addEnd(item); }
