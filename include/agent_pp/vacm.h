...
public:
    ...
    /**
     * Add a new security name to group mapping to the
     ...
     * @param[in] type        vacmViewTreeFamilyType (view_included or
     * view_excluded)
     * @param[in] storageType vacmViewTreeFamilyStorageType
     * (storageType_volatile, ...)
     *
     * @return true if the new row could be added. False, if it already exists.
     */
    bool addNewView(const NS_SNMP OctetStr& viewName, const Oidx& subtree,
     ...
     * configuration. This method is not synchronized. When called while the
     * VACM is potentially being used by other threads, lock the tables by
     * <pre>
     *    mib->lock_mib();
