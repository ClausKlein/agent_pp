...
mib_type MibGroup::type() const { return AGENTPP_GROUP; }
...
void MibGroup::load_from_file(const char* fname)
{
    ...
    for (cur.init(&content); ((cur.get()) && (size > 0)); cur.next(), n++)
    {
        ...
        {
            LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
            LOG("MibGroup: loading persistent data failed, cannot read length "
                "(file)(oid)(size)");
            LOG(fname);
...
bool MibContext::load_from(const OctetStr& p)
{
    ...
    for (cur.init(&groups); cur.get(); cur.next())
    {
        ...
        {
            // clear old data
            ListCursor<MibEntry> entries = cur.get()->get_content();
...
int MibContext::find(const Oidx& oid, MibEntryPtr& entry) TS_SYNCHRONIZED({
    ...
    MibEntry* MibContext::seek(const Oidx& oid) TS_SYNCHRONIZED({
        Oidx tmpoid(oid);
...
MibEntry* MibContext::add(MibEntry* item)
{
    ...
    {
        LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
        LOG("MibContext: cannot add not-accessible MIB object (oid) to "
            "(context)");
        LOG(item->key()->get_printable());
...
bool MibContext::remove_group(const Oidx& oid) TS_SYNCHRONIZED({
    Oidx      tmpoid(oid);
