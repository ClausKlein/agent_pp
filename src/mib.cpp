...
MibLeaf::MibLeaf(const Oidx& o, mib_access a, SnmpSyntax* s) : MibEntry(o, a)
{
    init(s, VMODE_NONE);
}

MibLeaf::MibLeaf(const Oidx& o, mib_access a, SnmpSyntax* s, int mode)
    : MibEntry(o, a)
{
    init(s, mode);
}
...
{
    return (unset() == SNMP_ERROR_SUCCESS) ? SNMP_ERROR_SUCCESS
...
Counter32MibLeaf* Counter32MibLeaf::get_instance(
    Mib* mib, const Oidx& oid, const Oidx& ind)
{
...
uint32_t Counter32MibLeaf::incrementScalar(
    Mib* mib, const Oidx& counterInstanceOid)
{
...
uint32_t Counter32MibLeaf::incrementColumnar(
    Mib* mib, const Oidx& columnOid, const Oidx& indexRow)
{
...
uint32_t Counter32MibLeaf::incrementColumnar(Mib* mib, const OctetStr& context,
    ...
    if (value)
    {
        ...
        case rowNotReady:
            return ((l == rowDestroy) || (l == rowActive)
                || (l == rowNotInService));
        case rowActive:
            return ((l == rowActive) || (l == rowNotInService)
                || (l == rowDestroy));
        case rowNotInService:
            return ((l == rowNotInService) || (l == rowActive)
                || (l == rowDestroy));
    ...
    if (undo)
    {
        ...
        case rowEmpty:
            if ((get() == rowActive) || (get() == rowCreateAndGo))
                my_table->fire_row_changed(
                    rowDestroy, my_row, my_row->get_index());
            if (value) delete value;
            value = undo;
            undo  = 0;
            break;
        ...
        case rowNotReady:
            if (get() == rowActive)
                my_table->fire_row_changed(
                    rowNotInService, my_row, my_row->get_index());
            if (value) delete value;
            value = undo;
            undo  = 0;
            break;
...
MibTable::MibTable(const MibTable& other) : MibEntry(other.oid, other.access)
{
    ...
    memcpy(
        index_struc, other.index_struc, sizeof(index_info) * other.index_len);
...
MibTable::MibTable(const Oidx& o, const index_info* istruc, unsigned int ilen)
    : MibEntry(o, NOACCESS)
{
    init(o, istruc, ilen);
}
...
bool MibTable::serialize(char*& buf, int& sz)
{
    ...
    int            len = size + 10;
    unsigned char* cp  = asn_build_header((unsigned char*)buf, &len,
...
bool MibTable::deserialize(char* buf, int& sz)
{
    ...
    while (size > 0)
    {
        ...
        buf                   = (char*)data;
        if ((status != SNMP_CLASS_SUCCESS) || (vbsz == 0)
            || (vbsz != generator.size()))
        {
...
bool MibTable::ready(Vbx* pvbs, int sz, MibTableRow* row)
{
    ...
    for (int i = 0; i < sz; i++)
    {
        ...
        if (generator.get_nth(i) != row_status)
        {
            ...
            if ((c->get_access() > READONLY) && (pvbs[i].valid())
    ...
    if ((row_status) && (!(row_timeout.in_time())))
    {
        ...
        for (cur.init(&notready_rows); cur.get();)
        {
            ...
            if (status->get() == rowNotReady)
            {
                ...
                LOG_END;
                fire_row_changed(
                    rowDestroy, cur.get(), cur.get()->get_index());
 ...
 */
void MibTable::get_contents(
    Vbx**& contents, int& rows, int& cols, int discriminator)
{
    start_synch();
    if (!contents)
    {
...
List<MibTableRow>* MibTable::get_rows_cloned(
    const Oidx* prefix, int discriminator)
{
    start_synch();
    OidListCursor<MibTableRow> cur;
    List<MibTableRow>*         list = new List<MibTableRow>();
...
Mib::Mib(
    const OctetStr& persistentObjectsPath, const OctetStr& bootCounterPath)
{
...
int Mib::notify(const OctetStr& context, const Oidx& oid, Vbx* vbs, int sz,
...
bool Mib::remove(const Oidx& oid)
{
    ...
    if (!defaultContext->remove_group(oid))
    {
        ...
        if ((defaultContext->find(oid, entry) == SNMP_ERROR_SUCCESS)
            && (entry))
        {
            entry->start_synch();
...
MibConfigFormat* Mib::add_config_format(
    unsigned int formatID, MibConfigFormat* format)
{
...
bool Mib::is_complex_node(const MibEntryPtr& entry)
{
    return ((entry->type() == AGENTPP_TABLE)
        || (entry->type() == AGENTPP_PROXY) || (entry->type() == AGENTX_NODE)
...
int Mib::find_managing_object(
    MibContext* context, const Oidx& oid, MibEntryPtr& retval, Request* req)
{
 ...
 */
int Mib::next_access_control(
    Request* req, MibEntry* entry, Oidx& oid, const Oidx& nextOid)
{
...
bool Mib::process_request(Request* req, int reqind)
{
    ...
    case (sNMP_PDU_GET): {
        ...
        // entry not available
#ifdef _SNMPv3
        if ((err = find_managing_object(
                 get_context(req->get_context()), tmpoid, entry, req))
            != SNMP_ERROR_SUCCESS)
#else
        ...
        // access control
        int vacmErrorCode =
            requestList->get_vacm()->isAccessAllowed(req->viewName, tmpoid);
    ...
    case (sNMP_PDU_GETNEXT): {
        ...
        do {
            nextOid.clear();
            if (find_next(get_context(req->get_context()), tmpoid, entry, req,
                    reqind, nextOid)
                != SNMP_ERROR_SUCCESS)
            {
#else
    reprocess:
        ...
        if (find_next(defaultContext, tmpoid, entry, req, reqind, nextOid)
            != SNMP_ERROR_SUCCESS)
        {
#endif
                unlock_mib();
            ...
            (vacmErrorCode = next_access_control(req, entry, tmpoid, nextOid))
            == VACM_notInView);
        if (vacmErrorCode != VACM_accessAllowed)
        {
            unlock_mib();
...
#ifdef _THREADS
    MibMethodCall* call =
        new MibMethodCall(this, &Mib::do_process_request, req);
#    ifdef AGENTPP_USE_THREAD_POOL
...
void Mib::unregister_proxy(
    const OctetStr& contextEngineID, ProxyForwarder::pdu_type type)
{
...
#else
            if ((err = find_managing_object(
                     defaultContext, tmpoid, entry, req))
                !=
#endif
                SNMP_ERROR_SUCCESS)
            {
...
#ifdef _SNMPv3
            // access control
            int vacmErrorCode = requestList->get_vacm()->isAccessAllowed(
                req->viewName, tmpoid);
            if (vacmErrorCode != VACM_accessAllowed)
            {
                unlock_mib();
    ...
    for (int j = 0; j < maxrep; j++)
    {
        ...
        for (; (id < req->subrequests()) && (id < endofNextRow); id++)
        {
            ...
            if (!req->is_done(id))
            {
                ...
                do {
                    nextOid.clear();
                    if (find_next(get_context(req->get_context()), tmpoid,
                            entry, req, id, nextOid)
                        !=
#else
            repeating:
                if (find_next(defaultContext, tmpoid, entry, req, id, nextOid)
...
#ifdef _SNMPv3
                } while ((vacmErrorCode = next_access_control(
                              req, entry, tmpoid, nextOid))
                    == VACM_notInView);
