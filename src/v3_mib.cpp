...
int UsmUserTableStatus::unset()
{
    if (undo)
    {
        ...
        case rowNotInService:
        case rowNotReady:
            if (get() == rowActive) deleteUsmUser();
            if (value) delete value;
            value = undo;
            undo  = nullptr;
            break;
...
UsmUserTable::UsmUserTable(v3MP* v3mp) : StorageTable(oidUsmUserEntry, iUsmUserTable, lUsmUserTable)
{
    ...
    for (int i = 1; i <= users; i++)
    {
        ...
        auto* ukc5 = (UsmKeyChange*)newRow->get_nth(5);
        auto* ukc6 = (UsmKeyChange*)newRow->get_nth(6);
        auto* ukc8 = (UsmKeyChange*)newRow->get_nth(8);
        auto* ukc9 = (UsmKeyChange*)newRow->get_nth(9);
...
void UsmUserTable::initialize_key_change(MibTableRow* row)
{
    ...
    auto* ukc5 = (UsmKeyChange*)row->get_nth(5);
    auto* ukc6 = (UsmKeyChange*)row->get_nth(6);
    auto* ukc8 = (UsmKeyChange*)row->get_nth(8);
    auto* ukc9 = (UsmKeyChange*)row->get_nth(9);
    ...
    const OctetStr& privKey, const bool add_to_usm)
{
    ...
    if (!newRow) newRow = add_row(newIndex);
            ...
            if (value_ok(req->get_value(ind)))
            {
                ...
                if (cloneRow) // check if row is active
                    if (cloneRow->get_nth(12))
                    {
                        ...
                        if (val == rowActive) return SNMP_ERROR_SUCCESS;
                    }
...
int UsmCloneFrom::set(const Vbx& vb)
{
    ...
    if (vb.valid() && (vb.get_oid() == get_oid()))
        if (vb.get_syntax() == get_syntax())
        {
            ...
            auto* ukc5 = (UsmKeyChange*)my_row->get_nth(5);
            auto* ukc6 = (UsmKeyChange*)my_row->get_nth(6);
            auto* ukc8 = (UsmKeyChange*)my_row->get_nth(8);
            auto* ukc9 = (UsmKeyChange*)my_row->get_nth(9);
...
{
    OctetStr os;
    if (vb.get_value(os) != SNMP_CLASS_SUCCESS) return false;
...
int usmUserPrivProtocol::prepare_set_request(Request* req, int& ind)
{
    ...
    if (o != oidUsmNoPrivProtocol)
    {
        ...
        if (my_row)
        {
            ...
            if (auth == oidUsmNoAuthProtocol) return SNMP_ERROR_INCONSIST_VAL;
        }
    }
