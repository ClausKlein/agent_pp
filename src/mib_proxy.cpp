...
OidxPtr MibProxy::max_key() { return &range; }
...
void MibProxy::get_request(Request* req, int reqind)
{
    ...
    if (get_access() >= READONLY)
    {
        ...
        status = SnmpRequest::get(source, &vb, 1, errind, community[READING]);
...
int MibProxy::commit_set_request(Request* req, int reqind)
{
    ...
    status = SnmpRequest::set(source, &vb, 1, errind, community[WRITING]);
...
OidxPtr MibProxyV3::max_key() { return &range; }
...
bool MibProxyV3::match_target_params(Request* req, const OctetStr& paramsIn)
{
    ...
    LOG_BEGIN(loggerModuleName, DEBUG_LOG | 6);
    LOG("MibProxyV3: matching "
        "(MPModel)(match)(secModel)(match)(secLevel)(match)");
...
int MibProxyV3::process_single(Pdux& pdu, Request* req)
{
    ...
    LOG_BEGIN(loggerModuleName, INFO_LOG | 3);
    LOG("MibProxyV3: get: contacting agent "
        "(address)(secName)(secLevel)(context)(contextEngineID)");
