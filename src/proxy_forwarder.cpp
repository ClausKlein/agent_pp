...
void ProxyForwarder::check_references(Mib* mib)
{
    ...
    if ((!_snmpProxyEntry) || (!_snmpTargetParamsEntry) || (!_snmpTargetAddrEntry))
    {
        ...
        LOG("ProxyForwarder: internal error: need SNMP-PROXY- and "
            "SNMP-TARGET-MIB in the Mib default context.");
        LOG_END;
    }
}
...
bool ProxyForwarder::process_multiple(Pdux& pdu, Request* req)
{
    ...
    for (cur.init(matches); cur.get(); cur.next())
    {
        ...
        for (tcur.init(targets); tcur.get(); tcur.next())
        {
            ...
            LOG("ProxyForwarder: contacting agent "
                "(type)(address)(secName)(secLevel)(context)("
                "contextEngineID)");
            LOG(pdu.get_type());
...
bool ProxyForwarder::process_single(Pdux& pdu, Request* req)
{
    ...
    target->get_address(addr);
    LOG_BEGIN(loggerModuleName, INFO_LOG | 3);
    LOG("ProxyForwarder: get: contacting agent "
