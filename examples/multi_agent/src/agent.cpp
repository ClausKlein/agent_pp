...
const table_size_def table_sizes[4] = { table_size_def(oidUsmUserEntry, 30),
    table_size_def(oidVacmSecurityToGroupEntry, 30), table_size_def(oidVacmAccessEntry, 30),
...
void SnmpAgent::run()
{
    ...
    if (!AgentTools::make_path(persistentObjectsPath.get_printable()))
    {
        ...
        LOG("Directory for storing persistent data could not be created "
            "(dir)");
    ...
    MibTableSizePolicy::register_policy(mib->get_default_context(), &policy);
#endif
